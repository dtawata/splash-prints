...
export const getFavorites = async (email) => {
  try {
    const queryString = 'select * from favorites where email = ?';
    const queryArgs = [email];
    const data = await connection.promise().query(queryString, queryArgs);
    return data[0];
  }
...
export const getRelated = async (path) => {
  try {
    const queryString = 'select prints.* from prints inner join related on related.print_id = prints.id and related.path = ?;';
    const queryArgs = [path];
    const data = await connection.promise().query(queryString, queryArgs);
    return data[0];
  }
